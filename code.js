// Main plugin code for Smart Find & Replace
...
function textMatches(text, searchText, options) {
...
function getTextNodes(node, selectionOnly) {
  const textNodes = [];
...
function getPreviewText(text, searchText, options) {
  const maxLength = 100;
  let preview = text;
...
// Main find function
async function findMatches(searchText, options) {
  try {
...
async function replaceMatches(searchText, replaceText, options) {
  try {
    const textNodes = getTextNodes(figma.currentPage, options.selectionOnly);
    let replaceCount = 0;
